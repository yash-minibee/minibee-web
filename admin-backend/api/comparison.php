...
if ($_SERVER['REQUEST_METHOD'] === 'GET' && $action === 'list') {
    $rows = $db->query("SELECT * FROM feature_comparison ORDER BY sort_order ASC, id ASC")->fetchAll(PDO::FETCH_ASSOC);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $action === 'update') {
    $stmt = $db->prepare("
        UPDATE feature_comparison SET
            feature=:feature, starter=:starter, growth=:growth,
            premium=:premium, enterprise=:enterprise, sort_order=:sort_order
        WHERE id=:id
    ");
    $stmt->execute([
        ':id'         => $input['id'],
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $action === 'delete') {
    $stmt = $db->prepare("DELETE FROM feature_comparison WHERE id=:id");
